...
public class AmoCrmLeadService {
    ...
    public void onDocumentPassedPrecheck(DocumentPassedPrecheckEvent event) {
        ...
        try {
            ...
            auditWriter.write(review, DocumentReviewAuditEventType.CRM_LEAD_CREATED, Map.of("leadId", leadId, "contactId", contactId));
            ...
            log.error("Failed to create amoCRM lead for documentReview {} — document stays PENDING_MANAGER with no CRM card; " +
                    "needs manual follow-up", review.getId(), e);
            auditWriter.write(review, DocumentReviewAuditEventType.CRM_LEAD_CREATED, Map.of("error", String.valueOf(e.getMessage())));
        }
    }
}
